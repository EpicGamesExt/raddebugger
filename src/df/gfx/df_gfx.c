...
df_append_value_strings_from_eval(Arena *arena, DF_EvalVizStringFlags flags, U32 default_radix, F_Tag font, F32 font_size, F32 max_size, S32 depth, E_Eval eval, E_Member *member, DF_CfgTable *cfg_table, String8List *out)
{
  ...
  B32 no_addr = (df_cfg_val_from_string(cfg_table, str8_lit("no_addr")) != &df_g_nil_cfg_val) && (flags & DF_EvalVizStringFlag_ReadOnlyDisplayRules);
  ...
  else if(max_size > 0) switch(e_type_kind_from_key(e_type_unwrap(eval.type_key)))
  {
    ...
    case E_TypeKind_Array:
    {
      ...
      if(!did_content && (flags & DF_EvalVizStringFlag_ReadOnlyDisplayRules))
      {
        did_content = 1;
  ...
  for(TXT_Token *token = tokens.v; token < tokens_opl; token += 1)
  {
    ...
    switch(token->kind)
    {
      ...
      case TXT_TokenKind_Identifier:
      {
        ...
        {
          DF_Entity *module = df_entity_from_handle(df_interact_regs()->module);
          DI_Key dbgi_key = df_dbgi_key_from_module(module);
          U64 symbol_voff = df_voff_from_dbgi_key_symbol_name(&dbgi_key, token_string);
          if(symbol_voff != 0)
          {

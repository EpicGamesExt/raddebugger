  ...
  artifacts:
  {
    ...
    b_obj:
    {
      ...
      coff: { object:
      {
        ...
        sections:
        {
          comdat: { name: ".rdata", permissions: (read), content: initialized_data, flags: (link_comdat), data: { concat: { text: "H", zero: 1 } } }
          regular:
          {
            name: ".rdata"
            permissions: (read)
            ...
            data: { zero: 4 }
            relocations: { static_ref: { type: Addr32Nb, offset: 0, symbol: static_symbol } }
          }
        }
        symbols:
        {
          comdat_def: { kind: section_definition, section: comdat, selection: Largest }
          test: { kind: external, name: "TEST", section: comdat, value: 1 }
          static_symbol: { kind: static, name: "STATIC", section: comdat, value: 2 }
        }
      } }
    }
    entry_obj:

test:
{
  artifacts:
  {
    ...
    large_obj:
    {
      ...
      coff: { object: { machine: x64,
        sections: { data: { name: ".data", permissions: (read, write), content: initialized_data, flags: (link_comdat), data: { text: "test" } } }
        symbols: {
  ...
  build:
  {
    ...
    link: { args: "/subsystem:console /entry:entry /out:a.exe communal.obj large.obj entry.obj", artifact: a_image }
    link: { args: "/subsystem:console /entry:entry /out:b.exe large.obj communal.obj entry.obj", artifact: b_image }
  }
  steps:
  {

test:
{
  artifacts:
  {
    a_obj: { file_name: "a.obj", coff: { object: { machine: x64, symbols: {
      ...
      w: { kind: weak, name: "w", fallback: q, search: anti_dependency }
    } } } }
    entry_obj:
    {
      file_name: "entry.obj"
      coff: { object: { machine: x64, sections: { text: {
        ...
        w: { kind: weak, name: "w", fallback: e, search: no_library }
      } } }
    }
    image: { file_name: "a.exe", pe: {} }
  }
  build: { link: { args: "/subsystem:console /entry:entry /out:a.exe entry.obj a.obj", artifact: image } }
  steps: { expect_pe: { artifact: image, expected: { pe: { sections: { ".text": { data: 48c7c022020000c3 } } } } } }
}

...
lnk_push_linker_symbols(Arena *arena, LNK_Config *config)
{
  ...
  coff_obj_writer_push_symbol_abs(obj_writer, str8_lit("__ImageBase"), base_addr, COFF_SymStorageClass_External);
...
lnk_build_base_relocs(TP_Context *tp, TP_Arena *tp_arena, LNK_Config *config, U64 objs_count, LNK_Obj **objs)
{
  ...
  LNK_BaseRelocPageArray page_arr;
  {
...

    {
      ...
      task.page_size            = config->page_size;
  ...
  if (page_arr.count) {
    ...
    HashTable *voff_ht = hash_table_init(scratch.arena, config->page_size);
    for (U64 page_idx = 0; page_idx < page_arr.count; ++page_idx) {
      ...
      for (U64Node *i = page->entries_addr32.first; i != 0; i = i->next) {
        ...
        Assert(rel_off <= config->page_size);
...
lnk_build_win32_image(TP_Arena *arena, TP_Context *tp, LNK_Config *config, LNK_SymbolTable *symtab, LNK_ObjList obj_list)
{
...

  {
    ...
    HashTable **sect_defn_ht_arr = push_array(temp.arena, HashTable *, tp->worker_count);
    for (U64 i = 0; i < tp->worker_count; i += 1) sect_defn_ht_arr[i] = hash_table_init(temp.arena, 128);
    HashTable *sect_defn_ht = sect_defn_ht_arr[0];
    ...
    for (U64 obj_idx = 0; obj_idx < objs_count; obj_idx += 1) {
      ...
      U32 obj_features = lnk_obj_get_features (obj);
      B32 discards_guards = (obj_features & MSCRT_FeatFlag_GUARD_CF) ||
        (obj_features & MSCRT_FeatFlag_GUARD_EH_CONT);
      ...
      for (U64 sect_idx = 0; sect_idx < obj->header.section_count_no_null; sect_idx += 1) {
        ...
        if (sect_defn == 0) {
          ...
          hash_table_push_string_raw(temp.arena, sect_defn_ht, sect_name_with_flags, sect_defn);
        }
    ...
    ProfBegin("Merge Section Definitions");
    HashTable *main_sect_defns_ht = sect_defn_ht_arr[0];
    for (U64 worker_idx = 1; worker_idx < tp->worker_count; worker_idx += 1) {
      U64                     sect_defns_count = sect_defn_ht_arr[worker_idx]->count;
      LNK_SectionDefinition **sect_defns       = values_from_hash_table_raw(temp.arena, sect_defn_ht_arr[worker_idx]);
      radsort(sect_defns, sect_defns_count, lnk_section_definition_is_before);

      for (U64 i = 0; i < sect_defns_count; i += 1) {
        String8                name_with_flags = lnk_make_name_with_flags(temp.arena, sect_defns[i]->name, sect_defns[i]->flags);
        LNK_SectionDefinition *main_defn       = 0;
        hash_table_search_string_raw(main_sect_defns_ht, name_with_flags, &main_defn);
        if (main_defn == 0) {
          main_defn = sect_defns[i];
          hash_table_push_string_raw(temp.arena, main_sect_defns_ht, name_with_flags, main_defn);
        } else {
          main_defn->contribs_count += sect_defns[i]->contribs_count;
        }
      }
    }
    ProfEnd();
    ...
    U64                     sect_defns_count = main_sect_defns_ht->count;
    LNK_SectionDefinition **sect_defns       = values_from_hash_table_raw(temp.arena, main_sect_defns_ht);
    radsort(sect_defns, sect_defns_count, lnk_section_definition_is_before);
  ...
  U64 expected_image_header_size;
  {
    ...
    LNK_SectionContrib ***sect_map = push_array(scratch.arena, LNK_SectionContrib **, objs_count);
    {
      U16 default_align = lnk_default_align_from_machine(config->machine);
      for (U64 obj_idx = 0; obj_idx < objs_count; obj_idx += 1) {
        LNK_Obj            *obj           = objs[obj_idx];
        COFF_SectionHeader *section_table = (COFF_SectionHeader *)str8_substr(obj->data, obj->header.section_table_range).str;
        String8             string_table  = str8_substr(obj->data, obj->header.string_table_range);

        sect_map[obj_idx] = push_array(scratch.arena, LNK_SectionContrib *, obj->header.section_count_no_null);
    ...
    // patch symbol tables
    {
      ...
      B8 **was_patched = push_array(scratch.arena, B8 *, objs_count);
      for (U64 obj_idx = 0; obj_idx < objs_count; obj_idx += 1) {
        was_patched[obj_idx] = push_array(scratch.arena, B8, objs[obj_idx]->header.symbol_count);
      }
      ...
      for (U64 contrib_idx = 0; contrib_idx < common_block_contribs_count; contrib_idx += 1) {
        ...
        was_patched[obj->input_idx][symbol->u.defined.symbol_idx] = 1;

        if (obj->header.is_big_obj) {
          COFF_Symbol32 *symbol32 = parsed_symbol.raw_symbol;
          symbol32->value          = contrib->u.offset;
          symbol32->section_number = safe_cast_u32(common_block_sect->sect_idx + 1);
        } else {
          COFF_Symbol16 *symbol16 = parsed_symbol.raw_symbol;
          symbol16->value          = contrib->u.offset;
          symbol16->section_number = safe_cast_u16(common_block_sect->sect_idx + 1);
        }
      }
      ProfEnd();
      ...
      for (U64 obj_idx = 0; obj_idx < objs_count; obj_idx += 1) {
        ...
        for (U64 symbol_idx = 0; symbol_idx < obj->header.symbol_count; symbol_idx += (1 + symbol.aux_symbol_count)) {
          ...
          if (was_patched[obj_idx][symbol_idx]) {
            continue;
          }
      ...
      // patch weak symbols with strong definition
      for (U64 obj_idx = 0; obj_idx < objs_count; obj_idx += 1) {
        LNK_Obj *obj = objs[obj_idx];
      ...
      // patch weak with fallback definition
      {
        ...
        for (U64 obj_idx = 0; obj_idx < objs_count; obj_idx += 1) {
          ...
          for (U64 symbol_idx = 0; symbol_idx < obj->header.symbol_count; symbol_idx += (1 + symbol.aux_symbol_count)) {
            ...
            if (interp == COFF_SymbolValueInterp_Undefined) {
              ...
              for (;;) {
                // lookup definition
                LNK_Symbol *defn = lnk_symbol_table_search(symtab, LNK_SymbolScope_Defined, lookup_name);
                if (defn == 0) {
                  break;
                }
      ...
      // patch weak symbols which have undefined tag
      {
        for (U64 obj_idx = 0; obj_idx < objs_count; obj_idx += 1) {
          LNK_Obj *obj = objs[obj_idx];
    ...
    for (U64 i = 0; i < sects.count; i += 1) {
      lnk_assign_section_virtual_space(sects.v[i], config->sect_align, &voff_cursor);
    }

    // patch virtual offset and size in section headers
    for (U64 obj_idx = 0; obj_idx < objs_count; obj_idx += 1) {
      LNK_Obj *obj = objs[obj_idx];
      COFF_SectionHeader *section_table = (COFF_SectionHeader *)str8_substr(obj->data, obj->header.section_table_range).str;
      for (U64 sect_idx = 0; sect_idx < obj->header.section_count_no_null; sect_idx += 1) {
    ...
    for (U64 i = 0; i < sects.count; i += 1) {
      lnk_assign_section_file_space(sects.v[i], &foff_cursor);
    }

    // patch file offset and size in section headers
    for (U64 obj_idx = 0; obj_idx < objs_count; obj_idx += 1) {
      LNK_Obj *obj = objs[obj_idx];
      COFF_SectionHeader *section_table = (COFF_SectionHeader *)str8_substr(obj->data, obj->header.section_table_range).str;
      for (U64 sect_idx = 0; sect_idx < obj->header.section_count_no_null; sect_idx += 1) {
...

  {
    ...
    // patch load config
    {
      ...
      if (load_config_symbol) {
        ...
        if (sizeof(load_config_size) <= load_config_data.size) {
          PE_DataDirectory *load_config_dir = str8_deserial_get_raw_ptr(image_data, pe.data_dir_range.min + sizeof(PE_DataDirectory)*PE_DataDirectoryIndex_LOAD_CONFIG, sizeof(PE_DataDirectory));
    ...
    {
      LNK_Symbol *dll_table_begin_symbol = lnk_symbol_table_search(symtab, LNK_SymbolScope_Defined, str8_lit("RAD_LINKER_IMPORT_TABLE_DLL_BEGIN"));
      LNK_Symbol *dll_table_end_symbol   = lnk_symbol_table_search(symtab, LNK_SymbolScope_Defined, str8_lit("RAD_LINKER_IMPORT_TABLE_DLL_END"));
      if (dll_table_begin_symbol && dll_table_end_symbol) {
        COFF_ParsedSymbol dll_table_begin_parsed = lnk_parsed_symbol_from_coff_symbol_idx(dll_table_begin_symbol->u.defined.obj, dll_table_begin_symbol->u.defined.symbol_idx);
        COFF_ParsedSymbol dll_table_end_parsed   = lnk_parsed_symbol_from_coff_symbol_idx(dll_table_end_symbol->u.defined.obj, dll_table_end_symbol->u.defined.symbol_idx);
        U64 dll_table_voff  = image_section_table[dll_table_begin_parsed.section_number]->voff + dll_table_begin_parsed.value;
        U64 dll_table_vsize = dll_table_end_parsed.value - dll_table_begin_parsed.value;
        ...
        import_dir->virt_off  = dll_table_voff;
        import_dir->virt_size = dll_table_vsize;
      }

      LNK_Symbol *iat_begin_symbol = lnk_symbol_table_search(symtab, LNK_SymbolScope_Defined, str8_lit("RAD_LINKER IMPORT_ADDRESS_TABLE_BEGIN"));
      LNK_Symbol *iat_end_symbol   = lnk_symbol_table_search(symtab, LNK_SymbolScope_Defined, str8_lit("RAD_LINKER_IMPORT_ADDRESS_TABLE_END"));
      if (iat_begin_symbol && iat_end_symbol) {
        COFF_ParsedSymbol iat_begin_parsed = lnk_parsed_symbol_from_coff_symbol_idx(iat_begin_symbol->u.defined.obj, iat_begin_symbol->u.defined.symbol_idx);
        COFF_ParsedSymbol iat_end_parsed    = lnk_parsed_symbol_from_coff_symbol_idx(iat_end_symbol->u.defined.obj, iat_end_symbol->u.defined.symbol_idx);
        U64 iat_voff  = image_section_table[iat_begin_parsed.section_number]->voff + iat_begin_parsed.value;
        U64 iat_vsize = iat_end_parsed.value - iat_begin_parsed.value;
        ...
        import_addr_dir->virt_off  = iat_voff;
        import_addr_dir->virt_size = iat_vsize;
      }
    }

    // patch delay import
    {
      LNK_Symbol *dll_table_begin_symbol = lnk_symbol_table_search(symtab, LNK_SymbolScope_Defined, str8_lit("RAD_LINKER_IMPORT_TABLE_DLL_DELAY_BEGIN"));
      LNK_Symbol *dll_table_end_symbol   = lnk_symbol_table_search(symtab, LNK_SymbolScope_Defined, str8_lit("RAD_LINKER_IMPORT_TABLE_DLL_DELAY_END"));
      if (dll_table_begin_symbol && dll_table_end_symbol) {
        COFF_ParsedSymbol dll_table_begin_parsed = lnk_parsed_symbol_from_coff_symbol_idx(dll_table_begin_symbol->u.defined.obj, dll_table_begin_symbol->u.defined.symbol_idx);
        COFF_ParsedSymbol dll_table_end_parsed   = lnk_parsed_symbol_from_coff_symbol_idx(dll_table_end_symbol->u.defined.obj, dll_table_end_symbol->u.defined.symbol_idx);
        U64 dll_table_voff  = image_section_table[dll_table_begin_parsed.section_number]->voff + dll_table_begin_parsed.value;
        ...
        PE_DataDirectory *delay_import_dir = str8_deserial_get_raw_ptr(image_data, pe.data_dir_range.min + sizeof(PE_DataDirectory)*PE_DataDirectoryIndex_DELAY_IMPORT, sizeof(PE_DataDirectory));
        delay_import_dir->virt_off  = dll_table_voff;
        delay_import_dir->virt_size = dll_table_vsize;
      }
    }
...
lnk_run(int argc, char **argv)
{
  enum State {
    ...
    State_InputImportObjs,
    State_InputExportObjs,
    State_InputResObj,
    State_InputLinkerObj,
    State_InputDebugDirObj,
    State_BuildImage,
    State_BuildImpLib,
...
#define state_list_pop(l) (l).first->state; SLLQueuePop((l).first, (l).last); (l).count -= 1
  ...
  B32                   input_import_objs                = 1;
  B32                   input_export_objs                = 1;
  B32                   input_res_obj                    = 1;
  B32                   input_linker_obj                 = lnk_do_debug_info(config);
  B32                   input_debug_dir_obj              = lnk_do_debug_info(config);
  ...
  for (;;) {
    while (state_list.count) {
      ...
      case State_PushLinkerSymbols: {
        ...
        LNK_InputObjList linker_objs = lnk_push_linker_symbols(scratch.arena, config);
        lnk_input_obj_list_concat_in_place(&input_obj_list, &linker_objs);
        ProfEnd();
      } break;
    ...
    if (input_import_objs) {
      input_import_objs = 0;
      state_list_push(scratch.arena, state_list, State_InputImportObjs);
      continue;
    }
    if (input_export_objs) {
      input_export_objs = 0;
      state_list_push(scratch.arena, state_list, State_InputExportObjs);
      continue;
    }
    if (input_res_obj) {
      input_res_obj = 0;
      state_list_push(scratch.arena, state_list, State_InputResObj);
      continue;
    }
    if (input_linker_obj) {
      input_linker_obj = 0;
      state_list_push(scratch.arena, state_list, State_InputLinkerObj);
      continue;
    }
    if (input_debug_dir_obj) {
      input_debug_dir_obj = 0;
      state_list_push(scratch.arena, state_list, State_InputDebugDirObj);
      continue;
    }

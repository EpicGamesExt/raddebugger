...
gnu_parse_rdebug(Arena *arena, B32 is_64bit, U64 first_rdebug_vaddr, GNU_MemRead *mem_read_func, void *mem_read_ud)
{
  ...
  for (U64 rdebug_vaddr = first_rdebug_vaddr, rdebug_next = 0; rdebug_vaddr != 0; rdebug_vaddr = rdebug_next) {
    ...
    if (is_64bit) {
      U64 rdebug_read_size = mem_read_func(rdebug_vaddr, sizeof(rdebug), &rdebug, mem_read_ud);
      if (rdebug_read_size != sizeof(rdebug)) { goto exit; }
    } else {
      GNU_RDebugInfo32 rdebug32;
      U64 rdebug_read_size = mem_read_func(rdebug_vaddr, sizeof(rdebug32), &rdebug32, mem_read_ud);
      if (rdebug_read_size != sizeof(rdebug32)) { goto exit; }
    }
    if (rdebug.r_version < 1) { goto exit; }

    if (rdebug.r_version > 1) {
      U64 r_next_size = mem_read_func(rdebug_vaddr + sizeof(rdebug), sizeof(rdebug_next), &rdebug_next, mem_read_ud);
...
gnu_parse_link_map_list(Arena *arena, B32 is_64bit, U64 first_link_map_vaddr, GNU_MemRead *mem_read_func, void *mem_read_ud)
{
  ...
  for (U64 link_map_vaddr = first_link_map_vaddr; link_map_vaddr != 0; link_map_vaddr = link_map.next_vaddr) {
    if (is_64bit) {
      U64 link_map_size = mem_read_func(link_map_vaddr, sizeof(link_map), &link_map, mem_read_ud);
      if (link_map_size != sizeof(link_map)) { goto exit; }
    } else {
      GNU_LinkMap32 link_map32 = {0};
      U64 link_map_size = mem_read_func(link_map_vaddr, sizeof(link_map32), &link_map32, mem_read_ud);

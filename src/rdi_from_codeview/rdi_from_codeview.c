...
internal U64
cv2r_end_of_cplusplus_container_name(String8 str)
{
...
internal int
cv2r_namespace_node_is_before(void *raw_a, void *raw_b)
{
  CV2R_NamespaceNode **a = (CV2R_NamespaceNode **)raw_a, **b = (CV2R_NamespaceNode **)raw_b;
  return (str8_compar(a[0]->string, b[0]->string, 0) < 0);
}

internal U64
cv2r_voff_from_soff(CV2R_Section *sections, U64 sections_count, U64 sec_num, U64 soff)
{
  U64 result = 0;
  if(0 < sec_num && sec_num <= sections_count)
  {
    result = sections[sec_num-1].voff + soff;
  }
  return result;
}
...
internal String8
cv2r_string_from_off(CV2R_StringTable *strtbl, U64 off)
{
...
internal CV2R_CompUnitContribution *
cv2r_comp_unit_contribution_from_voff__binary_search(CV2R_CompUnitContribution *contributions, U64 contributions_count, U64 voff)
{
  CV2R_CompUnitContribution *result = 0;
  if(contributions_count != 0)
  {
    U64 first_idx = 0;
    U64 last_idx = contributions_count-1;
    for(;first_idx < last_idx;)
    {
      U64 mid_idx = (last_idx + first_idx) / 2;
      U64 mid_voff_first = contributions[mid_idx].voff_first;
      U64 mid_voff_opl = contributions[mid_idx].voff_opl;
...
internal U32
cv2r_tpi_hash_from_data(String8 string)
{
  U32 result = 0;
  U8 *ptr = string.str;
...
cv2r_itypes_from_name(Arena *arena, CV2R_TPIHash *tpi_hash, CV_LeafParsed *leaf, String8 name, B32 compare_unique_name, U32 output_cap)
{
  CV_TypeIdArray result = {0};
  if(tpi_hash->bucket_count != 0)
  {
    Temp scratch = scratch_begin(&arena, 1);
    U32 hash = cv2r_tpi_hash_from_data(name);
    ...
    for(CV2R_TPIHashBlock *block = tpi_hash->buckets[bucket_idx];
        block != 0;
        block = block->next)
    {
...
internal RDI_Arch
cv2r_rdi_arch_from_cv_arch(CV_Arch arch)
{
  RDI_Arch result = 0;
  switch(arch)
  {
...
cv2r_rdi_language_from_cv_language(CV_Language language)
{
...
cv2r_rdi_from_cv_c13_checksum_kind(CV_C13ChecksumKind k)
{
...
internal void
cv2r_location_case_list_push_over_lvar_addr_range(Arena *arena, RDIM_LocationCaseList *loc_cases, RDIM_Location loc, Rng1U64 voff_range, CV_LvarAddrGap *gaps, U64 gap_count)
{
  //- rjf: emit location for ranges not coverd by gaps
  CV_LvarAddrGap *gap_ptr = gaps;
  U64 voff_cursor = voff_range.min;
  for(U64 i = 0; i < gap_count; i += 1, gap_ptr += 1)
  {
    U64 voff_gap_first = voff_range.min + gap_ptr->off;
    U64 voff_gap_opl   = voff_gap_first + gap_ptr->len;
    if(voff_cursor < voff_gap_first)
    {
      RDIM_Rng1U64 truncated_voff_range = {voff_cursor, voff_gap_first};
      rdim_location_case_list_push(arena, loc_cases, loc, truncated_voff_range);
    }
    voff_cursor = voff_gap_opl;
  }
  
  //- rjf: emit remaining range
  if(voff_cursor < voff_range.max)
  {
    RDIM_Rng1U64 remaining_voff_range = {voff_cursor, voff_range.max};
    rdim_location_case_list_push(arena, loc_cases, loc, remaining_voff_range);
  }
}
...
cv2r_convert(Arena *arena, CV2R_ConvertParams *params)
{
  Temp scratch = scratch_begin(&arena, 1);
  ...
  CV2R_Section *sections = params->sections;
  CV2R_StringTable *strtbl = params->strtbl;
  CV_LeafParsed *tpi_leaf = params->tpi_leaf;
  CV_LeafParsed *ipi_leaf = params->ipi_leaf;
  CV2R_TPIHash *tpi_hash = params->tpi_hash;
  CV2R_TPIHash *ipi_hash = params->ipi_hash;
  ...
  ProfScope("build link name map") if(comp_units_count != 0 && lane_idx() == 0)
  {
    // rjf: set up
    {
      link_name_map = push_array(scratch.arena, CV2R_LinkNameMap, 1);
      link_name_map->buckets_count = Max(1, symbol_count_prediction);
      link_name_map->buckets = push_array(scratch.arena, CV2R_LinkNameNode *, link_name_map->buckets_count);
    }
      ...
      for(CV_RecIter iter = {0}; cv_rec_next(sym->data, &sym->sym_ranges, 0, &iter);)
      {
        switch(iter.kind)
        {
          ...
          case CV_SymKind_PUB32:
          {
            ...
            U64 voff = cv2r_voff_from_soff(sections, sections_count, pub32->sec, pub32->off);
            
            // rjf: commit to link name map
            U64 hash = cv2r_hash_from_voff(voff);
            U64 bucket_idx = hash%link_name_map->buckets_count;
            CV2R_LinkNameNode *node = push_array(scratch.arena, CV2R_LinkNameNode, 1);
  ...
  //
  CV2R_SrcFileStubArray *unit_file_stubs = 0;
  ...
  ProfScope("gather all file paths")
  {
    ...
    ProfScope("do wide gather")
    {
        ...
        for(;;)
      {
        ...
        CV2R_SrcFileStubNode *first_src_file_stub = 0;
        CV2R_SrcFileStubNode *last_src_file_stub = 0;
        ...
        for(CV_RecIter iter = {0}; cv_rec_next(sym->data, &sym->sym_ranges, 0, &iter);)
        {
          switch(iter.kind)
          {
            ...
            case CV_SymKind_GPROC32:
            {
              ...
              base_voff = cv2r_voff_from_soff(sections, sections_count, proc32->sec, proc32->off);
            }break;
                ...
                for(;;)
                {
                  ...
                  if(last_file_off != max_U32 && last_file_off != curr_file_off)
                  {
                    ...
                    if(last_file_off + sizeof(CV_C13Checksum) <= file_chksms->size)
                    {
                      ...
                      seq_file_name = cv2r_string_from_off(strtbl, name_off);
                    ...
                    if(hit_path_node == 0)
                    {
                      ...
                      CV2R_SrcFileStubNode *stub_n = push_array(scratch2.arena, CV2R_SrcFileStubNode, 1);
        ...
        unit_file_stubs[unit_idx].v = push_array_no_zero(scratch.arena, CV2R_SrcFileStub, unit_file_stubs[unit_idx].count);
        {
          U64 idx = 0;
          for EachNode(n, CV2R_SrcFileStubNode, first_src_file_stub)
          {
  ...
  CV2R_SrcFileMap *src_file_map = 0;
                                                ...
                                                RDIM_SubsetFlag_InlineLineInfo))
  {
    ...
    ProfScope("set up table")
    {
      ...
      src_file_map = push_array(scratch.arena, CV2R_SrcFileMap, 1);
      for EachIndex(idx, comp_units_count)
      {
        total_path_count += unit_file_stubs[idx].count;
      }
      src_file_map->slots_count = total_path_count + total_path_count/2 + 1;
      src_file_map->slots = push_array(scratch.arena, CV2R_SrcFileNode *, src_file_map->slots_count);
    }
    ...
    ProfScope("fill table")
    {
      for EachIndex(idx, comp_units_count)
      {
        CV2R_SrcFileStubArray stubs = unit_file_stubs[idx];
        ...
        for EachIndex(stub_idx, stubs.count)
        {
          ...
          CV2R_SrcFileNode *src_file_node = 0;
          for(CV2R_SrcFileNode *n = src_file_map->slots[src_file_slot]; n != 0; n = n->next)
          {
          ...
          if(src_file_node == 0)
          {
            src_file_node = push_array(arena, CV2R_SrcFileNode, 1);
  ...
  ProfScope("convert unit header info")
  {
    //- rjf: set up outputs
                               ...
                               RDIM_SubsetFlag_InlineLineInfo))
    {
      ...
      ProfScope("wide fill") for(;;)
      {
        ...
        // rjf: extract unit ranges
        RDIM_Rng1U64ChunkList unit_ranges = {0};
        if(src_unit != 0)
        {
          unit_ranges = src_unit->ranges;
        }
        ...
        if(dst_unit != 0)
        {
          ...
          dst_unit->language      = cv2r_rdi_language_from_cv_language(src_unit_sym->info.language);
          dst_unit->line_table    = line_table;
          dst_unit->voff_ranges   = unit_ranges;
        }
          ...
          switch(kind)
          {
            ...
            case CV_LeafKind_STRUCTURE:
            {
              ...
              if(lf_struct->props & CV_TypeProp_FwdRef)
              {
                ...
                itype_fwd = cv2r_first_itype_from_name(tpi_hash, tpi_leaf, do_unique_name_lookup?unique_name:name, do_unique_name_lookup);
              }
            }break;
  ...
  ProfScope("types pass 2: produce per-itype itype chain (for producing dependent types first)")
  {
    ...
    if(params->subset_flags & RDIM_SubsetFlag_Types)
    {
      ...
      for EachInRange(idx, range)
      {
        ...
        {
          CV2R_TypeIdChain *c = push_array(scratch.arena, CV2R_TypeIdChain, 1);
          c->itype = itype;
        ...
        Temp scratch2 = scratch_begin(&scratch.arena, 1);
        CV2R_TypeIdChain start_walk_task = {0, itype};
        CV2R_TypeIdChain *first_walk_task = &start_walk_task;
        CV2R_TypeIdChain *last_walk_task = &start_walk_task;
        for(CV2R_TypeIdChain *walk_task = first_walk_task;
            walk_task != 0;
             ...
             range->hdr.size >= 2)
          {
            ...
            switch(kind)
            {
              ...
              case CV_LeafKind_MODIFIER:
              {
                ...
                {
                  CV2R_TypeIdChain *c = push_array(scratch2.arena, CV2R_TypeIdChain, 1);
                  c->itype = lf->itype;
  ...
  //
  CV2R_NamespaceNode **all_namespace_slots = 0;
  ...
  ProfScope("gather all unique namespaces from types")
  {
    ...
    for EachInRange(idx, range)
    {
      ...
      // rjf: unpack itype info
      CV_RecRange *range = &tpi_leaf->leaf_ranges.ranges[itype-tpi_leaf->itype_first];
      CV_LeafKind kind = range->hdr.kind;
      U64 header_struct_size = cv_header_struct_size_from_leaf_kind(kind);
      
      // rjf: gather names from types
      String8 name = {0};
      if(range->off+range->hdr.size <= tpi_leaf->data.size &&
         range->off+2+header_struct_size <= tpi_leaf->data.size &&
         range->hdr.size >= 2)
      {
        ...
        switch(kind)
        {
          ...
          case CV_LeafKind_UNION:
          {
            CV_LeafUnion *lf_struct = (CV_LeafUnion *)itype_leaf_first;
            U8 *numeric_ptr = (U8 *)(lf_struct + 1);
            CV_NumericParsed size = cv_numeric_from_data_range(numeric_ptr, itype_leaf_opl);
            U8 *name_ptr = (U8 *)numeric_ptr + size.encoded_size;
            name = str8_cstring_capped(name_ptr, itype_leaf_opl);
      ...
      // rjf: add namespaces from strings
      String8 name_before_templates = str8_prefix(name, str8_find_needle(name, 0, str8_lit("<"), 0));
      for(U64 scope_resolution_operator_pos = 0;
          scope_resolution_operator_pos < name_before_templates.size;
          scope_resolution_operator_pos = str8_find_needle(name_before_templates, scope_resolution_operator_pos+1, str8_lit("::"), 0))
      {
        ...
        if(namespace_fully_qualified_name.size != 0)
        {
          ...
          for(String8Node *n = namespace_slots[slot_idx]; n != 0; n = n->next)
          {
            if(str8_match(n->string, namespace_fully_qualified_name, 0))
            {
              already_exists = 1;
              break;
            }
    ...
    //- rjf: on lane 0 -> combine all unique namespaces to singular map
    {
      ...
      U64 total_namespace_count = 0;
      U64 *total_namespace_count_ptr = &total_namespace_count;
      LaneNamespaceTable *lane_namespace_tables = 0;
      if(lane_idx() == 0)
      {
        lane_namespace_tables = push_array(scratch2.arena, LaneNamespaceTable, lane_count());
      }
      lane_sync_u64(&total_namespace_count_ptr, 0);
      lane_sync_u64(&lane_namespace_tables, 0);
      lane_namespace_tables[lane_idx()].slots = namespace_slots;
      lane_namespace_tables[lane_idx()].slots_count = namespace_slots_count;
      ins_atomic_u64_add_eval(total_namespace_count_ptr, namespace_count);
      lane_sync();
      ...
      if(lane_idx() == 0)
      {
        ...
        all_namespace_slots = push_array(scratch.arena, CV2R_NamespaceNode *, all_namespace_slots_count);
        for EachIndex(l_idx, lane_count())
        {
          ...
          for EachIndex(slot_idx, tbl->slots_count)
          {
            for(String8Node *n = tbl->slots[slot_idx]; n != 0; n = n->next)
            {
              ...
              B32 already_exists = 0;
              for(CV2R_NamespaceNode *dst_n = all_namespace_slots[dst_slot_idx]; dst_n != 0; dst_n = dst_n->next)
              {
              ...
              if(!already_exists)
              {
                CV2R_NamespaceNode *dst_n = push_array(scratch.arena, CV2R_NamespaceNode, 1);
  ...
  //////////////////////////////////////////////////////////////
  //- rjf: types pass 4: construct all types from TPI
  //
  ...
  if(lane_idx() == 0) ProfScope("types pass 4: construct all root/stub types from TPI")
  {
...
#define p2r_type_ptr_from_itype(itype) ((itype_type_ptrs && (itype) < itype_opl) ? (itype_type_ptrs[(itype_fwd_map[(itype)] ? itype_fwd_map[(itype)] : (itype))]) : 0)
    ...
    if(params->subset_flags & RDIM_SubsetFlag_Types)
    {
      ...
      {
        for(CV2R_TypeIdChain *itype_chain = itype_chains[root_itype];
            itype_chain != 0;
            itype_chain = itype_chain->next)
        {
          ...
          B32 itype_is_basic = (itype < tpi_leaf->itype_first);
...
#define p2r_type_ptr_from_itype(itype) ((itype_type_ptrs && (itype) < tpi_leaf->itype_opl) ? (itype_type_ptrs[(itype_fwd_map[(itype)] ? itype_fwd_map[(itype)] : (itype))]) : 0)
    ...
    for EachInRange(slot_idx, range)
    {
      for(CV2R_NamespaceNode *n = all_namespace_slots[slot_idx]; n != 0; n = n->next)
      {
        ...
        CV_TypeId container_type_id = cv2r_first_itype_from_name(tpi_hash, tpi_leaf, container_string, 0);
...
#undef p2r_type_ptr_from_itype
  ...
  RDIM_NamespaceChunkList all_namespaces = {0};
  {
    ...
    for EachInRange(slot_idx, range)
    {
      ...
      U64 node_count = 0;
      for(CV2R_NamespaceNode *n = all_namespace_slots[slot_idx]; n != 0; n = n->next) { node_count += 1; }
      CV2R_NamespaceNode **nodes = push_array(scratch.arena, CV2R_NamespaceNode *, node_count);
      {
        U64 idx = 0;
        for(CV2R_NamespaceNode *n = all_namespace_slots[slot_idx]; n != 0; n = n->next, idx += 1)
        {
          nodes[idx] = n;
        }
      }
      ...
      {
        CV2R_NamespaceNode *n = nodes[node_in_slot_idx];
  ...
  struct ScopeNamespaceNode
  {
    ScopeNamespaceNode *next;
    String8 string;
    RDIM_Scope *scope;
  };
  typedef struct ScopeNamespaceList ScopeNamespaceList;
  struct ScopeNamespaceList
  {
    ScopeNamespaceNode *first;
    ScopeNamespaceNode *last;
  };
  RDIM_TypeChunkList *syms_typedefs = 0;
  ScopeNamespaceList *syms_scopes_that_are_namespaces = 0;
...
#define p2r_type_ptr_from_itype(itype) ((itype_type_ptrs && (itype) < itype_opl) ? (itype_type_ptrs[(itype_fwd_map[(itype)] ? itype_fwd_map[(itype)] : (itype))]) : 0)
                               ...
                               RDIM_SubsetFlag_Types))
    {
      ...
      for(;;)
      {
        ...
        RDIM_SymbolChunkList *sym_constants = &sym_unit->constants;
        ...
        {
          RDIM_Symbol *defrange_target = 0;
          U64 procedure_num = 0;
          U64 procedure_base_voff = 0;
          CV_ProcFlags proc_flags = 0;
          U64 regrel_idx = 0;
          RDIM_Symbol *curr_proc_symbol = 0;
          typedef struct CV2R_ScopeNode CV2R_ScopeNode;
          struct CV2R_ScopeNode
          {
            CV2R_ScopeNode *next;
            RDIM_Scope *scope;
          };
          CV2R_ScopeNode *top_scope_node = 0;
          CV2R_ScopeNode *free_scope_node = 0;
          ...
          for(CV_RecIter iter = {0}; cv_rec_next(sym->data, &sym->sym_ranges, 0, &iter);)
          {
            switch(iter.kind)
            {
              ...
              case CV_SymKind_END:
              {
                CV2R_ScopeNode *n = top_scope_node;
                if(n != 0)
                {
              ...
              case CV_SymKind_BLOCK32:
              {
                ...
                RDIM_Scope *scope = rdim_scope_chunk_list_push(arena, sym_scopes, sym_scopes_chunk_cap);
                {
                  ...
                  U64 voff_first = cv2r_voff_from_soff(sections, sections_count, block32->sec, block32->off);
                  if(voff_first != 0)
                  {
                ...
                {
                  CV2R_ScopeNode *node = free_scope_node;
                  if(node != 0) { SLLStackPop(free_scope_node); }
                  else { node = push_array_no_zero(scratch.arena, CV2R_ScopeNode, 1); }
                  node->scope = scope;
              ...
              case CV_SymKind_GDATA32:
              {
                ...
                U64 voff = cv2r_voff_from_soff(sections, sections_count, data32->sec, data32->off);
                ...
                if(!is_duplicate)
                {
                  ...
                  {
                    CV_TypeId cv_type_id = cv2r_first_itype_from_name(tpi_hash, tpi_leaf, container_name, 0);
              ...
              case CV_SymKind_GPROC32:
              {
                ...
                if(params->subset_flags & RDIM_SubsetFlag_Scopes)
                {
                  ...
                  U64 voff_first = cv2r_voff_from_soff(sections, sections_count, proc32->sec, proc32->off);
                  if(voff_first != 0)
                  {
                ...
                // rjf: determine if this procedure is a namespace used for types; if so, gather
                if(procedure_root_scope != 0)
                {
                  U64 hash = u64_hash_from_str8(name);
                  U64 slot_idx = hash%all_namespace_slots_count;
                  for(CV2R_NamespaceNode *n = all_namespace_slots[slot_idx]; n != 0; n = n->next)
                  {
                    if(str8_match(n->string, name, 0))
                    {
                      ScopeNamespaceNode *node = push_array(arena, ScopeNamespaceNode, 1);
                      node->scope = procedure_root_scope;
                      node->string = name;
                      SLLQueuePush(syms_scopes_that_are_namespaces[sym_idx].first,
                                   syms_scopes_that_are_namespaces[sym_idx].last,
                                   node);
                      break;
                    }
                  }
              ...
              case CV_SymKind_THUNK32:
              {
                ...
                U64 voff_first = cv2r_voff_from_soff(sections, sections_count, thunk32->sec, thunk32->off);
                U64 voff_opl = voff_first + thunk32->len;
              ...
              case CV_SymKind_DEFRANGE_REGISTER:
              {
                ...
                U64 range_voff_first = cv2r_voff_from_soff(sections, sections_count, range->sec, range->off);
                U64 range_voff_opl = range_voff_first + range->len;
                ...
                cv2r_location_case_list_push_over_lvar_addr_range(arena, &defrange_target->location_cases, loc, voff_range, gaps, gap_count);
              }break;
              ...
              case CV_SymKind_DEFRANGE_FRAMEPOINTER_REL:
              {
                ...
                RDIM_Location location = cv2r_location_from_addr_reg_off(arena, arch, fp_register_code, byte_size, byte_pos, var_off, extra_indirection);
                
                // rjf: emit locations over ranges
                cv2r_location_case_list_push_over_lvar_addr_range(arena, &defrange_target->location_cases, location, voff_range, gaps, gap_count);
              }break;
              ...
              case CV_SymKind_DEFRANGE_FRAMEPOINTER_REL_FULL_SCOPE:
              {
                ...
                RDIM_Rng1U64 voff_range = {0, max_U64};
                rdim_location_case_list_push(arena, &defrange_target->location_cases, loc, voff_range);
              }break;
              ...
              case CV_SymKind_DEFRANGE_REGISTER_REL:
              {
                ...
                RDIM_Location loc = cv2r_location_from_addr_reg_off(arena, arch, reg_code, byte_size, byte_pos, var_off, extra_indirection_to_value);
...
#undef p2r_type_ptr_from_itype
    ...
    for(CV_RecIter iter = {0}; cv_rec_next(sym->data, &sym->sym_ranges, 0, &iter);)
    {
      ...
      switch(iter.kind)
      {
        ...
        case CV_SymKind_PUB32:
        {
          ...
          {
            CV2R_CompUnitContribution *contrib = cv2r_comp_unit_contribution_from_voff__binary_search(params->comp_unit_contributions, params->comp_unit_contributions_count, voff);
            if(contrib != 0)
            {
  ...
  //////////////////////////////////////////////////////////////
  //- rjf: upgrade namespace nodes with scopes info, if they match a scope
  //
  if(lane_idx() == 0)
  {
  ...
  for EachNode(n, RDIM_NamespaceChunkNode, all_namespaces.first)
  {
    ...
    for EachInRange(n_idx, range)
    {
      ...
      if(container_name.size != 0)
      {
        ...
        U64 hash = u64_hash_from_str8(container_name);
        U64 slot_idx = hash%all_namespace_slots_count;
        for(CV2R_NamespaceNode *ns_n = all_namespace_slots[slot_idx]; ns_n != 0; ns_n = ns_n->next)
        {
  ...
  //////////////////////////////////////////////////////////////
  //- rjf: join extra types from units
  //
...
#undef p2r_type_ptr_from_itype
  ...
  RDIM_BakeParams result = {0};
  {
    ...
    {
      for EachIndex(idx, sections_count)
      {
        CV2R_Section *src_sec = &sections[idx];
        RDIM_BinarySection *sec = rdim_binary_section_list_push(arena, &binary_sections);
        sec->name       = src_sec->name;
        sec->flags      = src_sec->flags;
        sec->voff_first = src_sec->voff;
        sec->voff_opl   = src_sec->voff+src_sec->vsize;
        sec->foff_first = src_sec->foff;
        sec->foff_opl   = src_sec->foff+src_sec->fsize;
      }
    }

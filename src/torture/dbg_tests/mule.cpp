...
///   windows: {
///     skip: ""
///     compile: "/Z7 /c /I%SRC% /EHsc mule_main.cpp mule_inline.cpp"
///     compile: "/Z7 /c /I%SRC% mule_c.c"
///     compile: "/Z7 /c /O2 mule_o2.cpp"
///     compile: "/Z7 /c mule_module.cpp"
///     link:    "/DEBUG:FULL /out:mule_main.exe mule_main.obj mule_inline.obj mule_c.obj mule_o2.obj"
///     link:    "/DEBUG:FULL /DLL mule_module.obj"
///     launch:  "mule_main.exe"
///   }
/// }
...
control_flow_stepping_tests(void){
  {
    int a = 1;
    if (a < 1){ /// 1000: { run_to_line at step_over }
      a += 1;
    }
    if (a < 2){ /// 1001: { at step_over }
      a += 2;   /// 1002: { at step_over }
    }
  }
  
  {
    int a = 1; /// 1003: { at step_over }
    if (a < 1) /// 1004: { at step_over }
    {
      a += 1;
    }
    if (a < 2) /// 1005: { at step_over }
    {
      a += 2;  /// 1006: { at step_over }
    }
  }
  
  {
    int a = 1; /// 1007: { at step_over }
    if (a < 1) /// 1008: { at step_over }
      a += 1;
    if (a < 2) /// 1009: { at step_over }
      a += 2;  /// 1010: { at step_over }
  }
  
  {
    int a = 1;     /// 1011: { at step_over }
    int b = 2;     /// 1012: { at step_over }
    if (a <= b){   /// 1013: { at step_over }
      if (a == b){ /// 1014: { at step_over }
        b += 1;
      }
      else{
        a += 1;    /// 1015: { at }
  ...
  {
    int a = 1; /// 1016: { run_to_line at step_over }
    int b = 2; /// 1017: { at step_over }
    if (a <= b)
    {
      if (a == b) /// 1018: { at step_over }
      {
        b += 1;
      }
      else
  ...
  {
    int a = 1;    /// 1020: { run_to_line at step_over }
    int b = 2;    /// 1021: { at step_over }
    if (a <= b)   /// 1022: { at step_over }
      if (a == b) /// 1023: { at step_over }
      b += 1;
    else
      a += 1;     /// 1024: { at step_over }
    else
      if (a%2)
      a = b;
    else
  ...
  {
    int x = 0;                       /// 1025: { at step_over }
    for (int i = 0; i < 10; i += 1){ /// 1026: { at step_over }
      x += i;                        /// 1027: { at step_over }
    }
  }
  
  {
    int x = 0; // 1028: { run_to_line at }

...
typedef struct RDI_Parsed
{
  ...
  RDI_NameMapBucket   *name_maps_buckets;
  RDI_U64              name_maps_bucket_count;
  RDI_NameMapNode     *name_maps_nodes;
  RDI_U64              name_maps_node_count;
  ...
  RDI_DataSection sections_by_tag[RDI_DataSectionTag_PRIMARY_COUNT];
...
typedef enum
{
  ...
  RDI_ParseStatus_OutOfData                 = 4,
  RDI_ParseStatus_Unknown                   = 5,
  RDI_ParseStatus_InvalidDataSectionTag     = 6,
  RDI_ParseStatus_InvalidSectionEncoding    = 7,
} RDI_ParseStatus;
...
static RDI_NameMap        rdi_name_map_nil        = {0};
static RDI_NameMapBucket  rdi_name_map_bucket_nil = {0};
static RDI_NameMapNode    rdi_name_map_node_nil   = {0};
#endif
...
RDI_PROC RDI_U32 *       rdi_idx_run_from_first_count(RDI_Parsed *rdi, RDI_U32 first, RDI_U32 raw_count, RDI_U32 *n_out);

//- table lookups

#define rdi_element_from_idx(rdi, name, idx) ((0 <= (idx) && (idx) < (rdi)->name##_count) ? &(rdi)->name[idx] : (rdi)->name ? &(rdi)->name[0] : 0)

//- line info

RDI_PROC void    rdi_parse_line_info(RDI_Parsed *rdi, RDI_U64 line_info_idx, RDI_ParsedLineInfo *out);
RDI_PROC void    rdi_line_info_from_unit(RDI_Parsed *rdi, RDI_Unit *unit, RDI_ParsedLineInfo *out);
RDI_PROC RDI_U64 rdi_line_info_idx_from_voff(RDI_ParsedLineInfo *line_info, RDI_U64 voff);

RDI_PROC void      rdi_parse_line_number_map(RDI_Parsed *rdi, RDI_U32 line_number_map_idx, RDI_ParsedLineMap *out);
RDI_PROC void      rdi_line_map_from_source_file(RDI_Parsed *rdi, RDI_SourceFile *src_file, RDI_ParsedLineMap *out);
...
RDI_PROC RDI_NameMap *     rdi_name_map_from_kind(RDI_Parsed *rdi, RDI_NameMapKind kind);
RDI_PROC void              rdi_name_map_parse(RDI_Parsed *rdi, RDI_NameMap *mapptr, RDI_ParsedNameMap *out);
RDI_PROC RDI_NameMapNode * rdi_name_map_lookup(RDI_Parsed *rdi, RDI_ParsedNameMap *map, RDI_U8 *str, RDI_U64 len);
RDI_PROC RDI_U32 *         rdi_matches_from_map_node(RDI_Parsed *rdi, RDI_NameMapNode *node, RDI_U32 *n_out);

//- checksums

RDI_PROC RDI_U64 rdi_checksum_from_offset(RDI_Parsed *rdi, RDI_U64 checksum_offset, RDI_ParsedChecksum *out);
RDI_PROC RDI_U64 rdi_time_stamp_from_parsed_checksum(RDI_ParsedChecksum checksum);

//- common helpers
RDI_PROC RDI_U64 rdi_first_voff_from_proc(RDI_Parsed *rdi, RDI_U32 proc_id);
